  ...
  persist(
    (set) => ({
      ...
      toggleDisease: (rowIndex, disease) =>
        set((state) => {
          const newData = [...state.csvData]
          const currentDiseases = newData[rowIndex].diseases || []

          if (currentDiseases.includes(disease)) {
            newData[rowIndex].diseases = currentDiseases.filter((d) => d !== disease)
          } else {
            newData[rowIndex].diseases = [...currentDiseases, disease]
          }
